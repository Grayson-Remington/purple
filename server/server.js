...
io.on("connection", (socket) => {
  socket.on("joinRoom", (data) => {
    ...
    if (rooms[roomId].players.some((player) => player.name === playerName)) {
      ...
      io.to(socket.id).emit(
        "nextCard",
        rooms[roomId].nextSuite + "_" + rooms[roomId].nextNumber.toString()
      );
      io.to(socket.id).emit(
        "thirdCard",
        rooms[roomId].thirdSuite + "_" + rooms[roomId].thirdNumber.toString()
      );
  ...
  socket.on("playAgain", () => {
    ...
    if (
      rooms[roomId].voteTotal >= Math.ceil(rooms[roomId].players.length / 2)
    ) {
      rooms[roomId].currentPlayerIndex = 0;
  ...
  socket.on("pass", (turnScore) => {
    ...
    rooms[roomId].currentPlayer =
      rooms[roomId].players[rooms[roomId].currentPlayerIndex].name;
    io.to(roomId).emit("turn", players[rooms[roomId].currentPlayerIndex].name);
  });
  ...
  socket.on("disconnect", () => {
    ...
    if (rooms[roomId]) {
      ...
      if (rooms[roomId].players.length === 0) {
        ...
        if (rooms[roomId].players.indexOf(rooms[roomId].currentPlayer) == -1) {
          if (
          ...
          rooms[roomId].currentPlayerIndex = rooms[roomId].players.indexOf(
            rooms[roomId].currentPlayer
          );
          if (
            rooms[roomId].currentPlayerIndex ==
